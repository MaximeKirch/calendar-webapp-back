...
exports.signupWorker = (req,res,next) => {
  ...
  bcrypt
    ...
    .then((hash) => {
      ...
      worker
        .save()
        .then(() => res.status(201).json({ message: "Worker registred !"}))
        .catch((err) => res.status(400).json({err}))
    })
...
exports.loginWorker = (req,res, next) => {
  const {email, password} = req.body
      ...
      bcrypt
        ...
        .then((valid) => {
          ...
          res
            ...
            .cookie(
              'jwt',
              jwt.sign({ workerId : worker._id}, process.env.TOKEN_SECRET, {
                expiresIn : '24h',
              })
            )
            .json({
              workerId: worker._id,
              token: jwt.sign({ workerId : worker._id}, process.env.TOKEN_SECRET, {
                expiresIn : '24h',
              })
            })
        })
